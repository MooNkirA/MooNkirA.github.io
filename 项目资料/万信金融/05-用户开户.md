...
### 解决 Hmily 与 Feign 冲突报错 NullPointerException
...
<span style="color: red;">解决方法：consumer-service 项目中定义一个 Spring 的后置处理器，将所有标识了 `@FeignClient` 注解的接口中没有被 `@Hmily` 注解修饰的方法，移除 `HmilyFeignInterceptor`</span>。具体实现如下：
...
> <span style="color: purple;">**值得注意的一点，这里是在构造方法中进行 RocketMQ 消费者的初始化，注意使用 `@Value` 注解进行依赖注入到类属性的值会为 null 的问题。**（具体问题分析在代码注释中）</span>
