...
- **token**：Spring Security 的 token 对象(<span style="color: red;">_需要注意，此对象不是认证的令牌_</span>)，里面存放了接入方调用 UAA 接口进行认证时传入的用户名、密码等需要验证的信息
...
> <span style="color: violet;">**注意：这里使用 postman 测试时，如果在 body 中使用 raw 类型的请求参数，会失败！必须使用 x-www-form-urlencoded **</span>
