...
##### ArrayList 的 contains 方法判断元素（自定义类型）是否存在的原理
...
#### 哈希表的存储元素详细过程(HashSet 判断元素唯一的原理)
...
#### HashSet 存储判断元素（自定义类型）重复的原理

##### HashSet 的 add/contains 等方法判断元素是否重复原理
...
- 尝试获取 key 对应的值，如果未获取到，则返回默认值。(1.8 版本后新方法)

```java
default void forEach(BiConsumer<? super K, ? super V> action)
```

- 遍历 map 的数据，可以直接使用 entry 中的 key 和 value。(1.8 版本后新方法)

```java
default V merge(K key, V value, BiFunction<? super V, ? super V, ? extends V> remappingFunction)
```

- 方法接收 3 个参数：key、value、function。如果 key 存在，将 value 按照 function 做 1 次计算后，更新到 Map 中；如果 key 不存在，将 key-value 放入 Map 中。(1.8 版本后新方法)

```java
default V putIfAbsent(K key, V value)
```

- 不存在 key 或者值为 null 时，才将键值对放入 Map。跟 put 方法相比，则方法不会直接覆盖已有的值，在不允许覆盖旧值的场景使用起来会比较简洁。(1.8 版本后新方法)

```java
default V compute(K key, BiFunction<? super K, ? super V, ? extends V> remappingFunction)
```

- 方法传入 2 个参数：key、function。主要处理流程是：获取到 key 对应的 oldValue（可能为 null）；经过 function 计算获取 newValue，将执行 put 方法，设置到 Map 中。(1.8 版本后新方法)

```java
default V computeIfAbsent(K key, Function<? super K, ? extends V> mappingFunction)
```

- 方法传入 2 个参数：key、function。是 `compute` 的衍生方法，此方法只在 key 不存在的时候，才执行 function 计算并返回 newValue；如果 key 对应的 value 存在，就直接返回原来的 value。

```java
default V computeIfPresent(K key, BiFunction<? super K, ? super V, ? extends V> remappingFunction)
```

- 与 `computeIfAbsent` 方法的逻辑相反，方法传入 2 个参数：key、function。此方法只在 key 存在时，才执行 function 计算并更新 newValue。

```java
default V replace(K key, V value)
```

- 如果 key 存在，则更新值；如果 key 不存在，则不作处理；

```java
default boolean replace(K key, V oldValue, V newValue)
```

- 如果 key 存在，并且值等于 oldValue 时，则设置新值；否则不作处理；

```java
default void replaceAll(BiFunction<? super K, ? super V, ? extends V> function)
```

- 对 Map 所有 k-v 执行 function 的处理获取 newValue，替换原来的 key 的值
