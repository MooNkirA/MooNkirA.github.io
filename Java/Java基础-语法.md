...
> Notes: <span style="color: red;">**main 是特殊的方法名，但不是关键字**</span>
...
char 类型是一个单一的 16 位 Unicode 字符。最小值是 `\u0000`（十进制等效值为 0）；最大值是 `\uffff`（即为 65535）。<span style="color: red;">**字符是使用`''`单引号包裹**</span>。

```java
char letter = 'A';
```

**补充说明**：unicode 编码占用两个字节，所以 <span style="color: red;">**char 类型的变量也是占用两个字节**</span>。
...
#### char 类型能否转成 int、String、double 类型
...
- protected：任意包下任意子类或同包下的任意类可以访问，可以修饰变量、方法。<span style="color: red;">**注意：不能修饰类（外部类）**</span>。
- 默认(包权限，不使用任何修饰符)：同包下的任意类都可以访问。可以修饰在类、接口、变量、方法。
- private：可以修饰变量、方法，在同一类内可见。<span style="color: red;">**注意：不能修饰类（外部类）**</span>
...
- 左边如果小于右边，结果是左边。
- 左边如果等于右边，结果是0。
- 正负号跟左边一致。即<span style="color: red;">负数取余也是负数</span>。
...
<span style="color: red;">**`static` 关键字的主要意义是，在于创建独立于具体对象的域变量或者方法。以致于即使没有创建对象，也能使用属性和调用方法！还有一个比较关键的作用就是，用来形成静态代码块以优化程序性能。**</span>
...
3. <span style="color: red;">**static 修饰的成员变量，在类的加载过程中，JVM 只为静态变量分配一次内存空间，以后创建该类对象的时候不会重新分配。而多个静态变量的初始化顺序是按照定义顺序来进行**</span>。
...
### static 修饰类【只能修饰内部类也就是静态内部类】（待整理）
...
> Notes: <span style="color: red;">**类中所有的 `private` 方法都隐式地指定为 `final`。**</span>
...
> Tips: <span style="color: red;">**枚举治理和常量治理结合使用，可能才是系统开发时的最佳实践**</span>
