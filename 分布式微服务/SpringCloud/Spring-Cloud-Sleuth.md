...
Spring Cloud Sleuth 主要功能就是在分布式系统中提供调用链追踪的解决方案，Sleuth 对用户透明，服务调用的交互信息都能被<span style="color: red;">**自动采集**</span>，可以通过日志文件获取服务调用的链路数据，也可以将数据发送给远程服务统一收集分析。
...
- **调用链跟踪解决异常问题思路**：一次完整的请求包含以上4个关键点，因此只需要<span style="color: red;">跟踪每个关键点</span>，便可知调用是否正常。缺少了哪个阶段的跟踪记录，就说明这个阶段异常了。
- **调用链跟踪解决性能问题思路**：<span style="color: red;">记录每个关键阶段的时间戳</span>，就可以计算任意阶段的耗时。
