...
1. **必须使用 管理员 CMD**（普通权限报错）
2. PowerShell 也可使用，但语法略有区别
...
- 无参数：创建 **文件符号链接**
- `/D`：创建 **目录符号链接** (Directory Symbolic Link)
- `/J`：创建 **目录联接** (Junction，日常最优解)
- `/H`：创建 **文件硬链接** (Hard Link)
...
- **路径有空格必须加英文双引号 `"` **
- **创建前，链接文件夹必须不存在**（即不能自己新建）
- **真实目录必须存在**
- `/J` 目录联接 **不支持网络路径**
- `/H` 硬链接仅支持同磁盘**
- <span style="color: red;">**致命禁忌：不要用 `del` 命令删除目录链接！！！**</span>此方式会<span style="color: red;">**直接清空源目录所有真实文件**</span>，造成数据丢失！正确删除方式如下：
    - **目录链接 /J /D**：使用 `rd` 命令。例如：`rd "C:\WeType"`
    - **文件链接 `/H`（或者无参数）**：直接 `del` 命令即可。删除链接 **只会删掉虚拟入口，不会动真实文件**。

#### 四种链接类型对比

`mklink` 一共只有 **4 种创建模式**

| 参数   | 类型                   | 适用对象 | 跨磁盘    | 跨网络    | 原文件删除后 |
| ------ | -------------------- | -------- | ------- | -------- | ----------- |
| 无参数 | **文件符号链接**       | 单个文件 | ✅ 支持   | ✅ 支持   | 失效         |
| `/D`   | **目录符号链接**       | 文件夹   | ✅ 支持   | ✅ 支持   | 失效         |
| `/J`   | **目录联接(Junction)** | 文件夹   | ✅ 支持   | ❌ 不支持 | **依然可用** |
| `/H`   | **文件硬链接**         | 单个文件 | ❌ 不支持 | ❌ 不支持 | 依然可用     |
