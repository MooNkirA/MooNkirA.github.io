...
有些不重要不需要同步的文件，但又需要放到当前设置了同步的文件夹中，此时可以设置忽略同步该文件/文件夹。

#### 【待验证】本地忽略文件

**本地忽略文件 `.nutstoreignore` **：针对指定文件夹灵活高效

在同步文件夹内创建特殊文件 `.nutstoreignore`，实现类似 Git 的忽略规则，**多端同步生效**。操作步骤：

- 在目标同步文件夹中，创建名为 `.nutstoreignore` 的文本文件（注意前面的点）
- 用文本编辑器打开，按以下规则编写忽略内容（每行一条规则）
- 保存文件后，坚果云会自动扫描并应用规则
...
| 规则格式      | 作用                           | 示例                             |
| ------------- | ------------------------------ | -------------------------------- |
| `文件名`      | 忽略当前目录下的指定文件       | `Thumbs.db`、`.DS_Store`         |
| `*.后缀`      | 忽略所有该后缀的文件           | `*.tmp`、`*.swp`、`*.log`        |
| `文件夹/`     | 忽略指定文件夹（递归）         | `node_modules/`、`cache/`        |
| `路径/文件名` | 忽略特定路径下的文件           | `.obsidian/workspace.json`       |
| `!文件名`     | 取消忽略（优先级高于其他规则） | `!important.txt`（不忽略此文件） |
...
**全局忽略规则 `customExtRules.cfg`：**全客户端生效

 Windows 系统配置全局忽略规则，对**所有同步文件夹**生效，适用于系统临时文件、编译产物等通用忽略需求。

1. 打开资源管理器，输入路径：`%APPDATA%\Nutstore\db`。（注：不一定是此路径，建议通过 Everything 直接搜索配置文件名）
2. 找到并编辑 `customExtRules.conf` 文件
3. 按格式添加忽略规则，保存后重启坚果云客户端
...
- **规则优先级**：文件夹级忽略 > 本地`.nutstoreignore` > 全局 `customExtRules.cfg`
- **生效范围**：
    - 文件夹级忽略：仅对当前文件夹生效，不随文件同步到其他设备
    - `.nutstoreignore`：随文件夹同步，多端统一生效
    - 全局规则：仅对当前设备的所有同步文件夹生效
- **特殊文件**：坚果云默认忽略常见临时文件（如 `.part`、`.tmp`、`.swp`、`#文件名#` 等），无需手动添加坚果云
- **规则生效**：修改忽略规则后，坚果云会自动重新扫描文件，无需手动触发同步
- **取消忽略**：在对应设置界面删除规则，或在.nutstoreignore 中添加 `!文件名` 规则

常见问题排查：

- **忽略规则不生效**：
    - 检查路径是否正确（相对路径 / 绝对路径是否匹配）
    - 确保规则格式正确（无多余空格、换行符）
    - 重启坚果云客户端刷新规则
