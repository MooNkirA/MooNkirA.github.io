...
- **以双下划线开头的 `__方法名` 为<span style="color: red;">真正的</span>私有方法**：类外部无法直接访问，Python 自动做名字重整，仅类内部可正常调用。
- **以单下划线开头的 `_方法名` 为<span style="color: red;">约定的</span>私有方法**：语法上可外部访问，仅开发者约定「不要随意改动」。
...
- **参数**：
    ...
    - `value`：要设置的新属性值
- **返回值**：无。
- **注意事项**：<span style="color: red;">**setter 方法名必须与 `@property` 方法名完全一致**</span>。
...
方式 1：**直接显式调用父类构造方法（最直观、最常用、推荐新手）**。在子类 `__init__` 中，**逐个手动调用**每个父类的初始化方法，并且注意<span style="color: red;">**必须手动传入 `self` 作为第一个参数**</span>。
...
**以单下划线开头的 `_属性名` 为<span style="color: red;">约定的</span>私有属性**，又称“保护属性”，特点是：
...
**以双下划线开头的 `__属性名` 为<span style="color: red;">真正的</span>私有属性**，Python 会自动对双下划线属性进行**名字重整**格式为 `_类名__attr`，让外部无法通过原名称直接访问，仅类内部可正常调用。其特点是：
